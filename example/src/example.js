...
class UITransaction extends React.Component{
	render(){
		...
		let k = classNames("mdl-text mdl-typography--subhead", {
		...
		return  <div className='AppTransaction--List'>
					<Layout horizontal={true} alignItems={'center'}>
						<FlexCell>
							...
							<div className="mdl-text mdl-text-normal-case mdl-typography--caption">{info.reason || 'Friendly Transfer'}</div>
						</FlexCell>
						<FixedCell>
							<h2 className={k}>$ {info.amount.toFixed(2)}</h2>
							<p className="mdl-text mdl-typography--caption-color-contrast mdl-typography--text-right">{(new Date(info.date)).toString().substring(4, 10)}</p>
						</FixedCell>
					</Layout>
				 </div>;
	}
					...
					<FlexCell fillFix={true}>
						...
						<div className={cnamefab}>
							<UIButtonView 
								...
								colored={true}
								ripple={true} 
							className="primary-fab"><i className="material-icons">person_add</i></UIButtonView>
						</div>
					</FlexCell>
				</Layout>;	
	}
